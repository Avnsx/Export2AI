...
| File | Size | Role |
|------|------|------|
| `package.slim.json` | ~22.6 KB | **Source of truth** — edit settings, commands, npm scripts |
| `package.json` | ~22.6 KB after `slim:package`; **~40.5 KB** after compile | Working manifest; produced by merge as base + model-target commands |
...
After local compile, run `npm run slim:package` to refresh both `package.slim.json` and `package.json` for git (strips generated commands/menus):
...
| `npm run test:soft-delete` | Repository control/context preservation, cache and secret excludes, keyword-safe source/workflow exceptions, manifest redaction, unreadable-path placeholders, and `.git` traversal guard |
...
| `npm run test:marketplace-assets` | Verifies `build/*.vsix` embeds marketplace assets and does not package generated context zips |
...
**Does not ship:** `src/`, `scripts/`, project `tests/`, dependency test folders, `package.slim.json`, `tsconfig.json`, `build/`, `*.vsix`, generated context zips (`*-chatgpt-context-*.zip`, `*-*-context-*.zip`), test zips
...
- **`package.json` is small** — ~22.6 KB after `slim:package`, ~40.5 KB after compile; if it balloons into the MB range, you reintroduced a generated-command explosion
